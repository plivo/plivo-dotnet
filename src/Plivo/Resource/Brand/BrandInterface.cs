...
namespace Plivo.Resource.Brand
{
    ...
    public class BrandInterface : ResourceInterface
    {
        ...
        public ListResponse<Brand> List(
            uint? type = null, uint? status = null)
        {
			...
			return ExecuteWithExceptionUnwrap(() =>
			{
				var resources = Task.Run(async () => await ListResources<ListResponse<Brand>>("10dlc/Brand",data).ConfigureAwait(false)).Result;
        ...
        /// <param name="type">type.</param>
        /// <param name="status">status.</param>
        public async Task<ListResponse<Brand>> ListAsync(
           uint? type = null, uint? status = null)
        {
            var mandatoryParams = new List<string> { "" };
            var data = CreateData(
            ...
            var resources = await ListResources<ListResponse<Brand>>("10dlc/Brand", data);
            resources.Objects.ForEach(
                (obj) => obj.Interface = this
            );
		...
		return ExecuteWithExceptionUnwrap(() =>
		{
				var result = Task.Run(async () => await Client.Update<Brand>(Uri + "10dlc/Brand/", data).ConfigureAwait(false)).Result;
       ...
       /// <returns>The create.</returns>
        ...
        var data = CreateData(
			...
			var result = await Client.Update<Brand>(Uri + "10dlc/Brand/", data);
            result.Object.StatusCode = result.StatusCode;
			return result.Object;
		}
		#endregion
