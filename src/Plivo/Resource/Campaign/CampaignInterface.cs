...
namespace Plivo.Resource.Campaign
{
    ...
    public class CampaignInterface : ResourceInterface
    {
        ...
        public ListResponse<Campaign> List(
            uint? brand = null, uint? usecase = null)
        {
			...
			return ExecuteWithExceptionUnwrap(() =>
			{
				var resources = Task.Run(async () => await ListResources<ListResponse<Campaign>>("10dlc/Campaign",data).ConfigureAwait(false)).Result;
        ...
        /// <param name="brand">brand.</param>
        public async Task<ListResponse<Campaign>> ListAsync(
           uint? brand = null, uint? usecase = null)
        {
            var mandatoryParams = new List<string> { "" };
            var data = CreateData(
            ...
            var resources = await ListResources<ListResponse<Campaign>>("10dlc/Campaign", data);
            resources.Objects.ForEach(
                (obj) => obj.Interface = this
            );
        ...
        {
        var mandatoryParams = new List<string>{"brand_id", "vertical", "usecase"};
        var data = CreateData(
            mandatoryParams,
            new
            {
       ...
       /// <returns>The create.</returns>
		...
		public async Task<Campaign> CreateAsync(
			string brand_id, string vertical, string campaign_alias, string usecase, string[] sub_usecases,
            string description, bool embedded_link,  bool embedded_phone, bool age_gated,
            bool direct_lending, bool subscriber_optin, bool subscriber_optout, bool subscriber_help, string sample1, string sample2)
		{
